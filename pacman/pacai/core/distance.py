...
def maze(position1, position2, gameState, isRed, width):
    """
    ...
    if isRed and x2 > width/2:
        position2 = (width/2)-3,y2

    if not isRed and x2 <= width/2:
        position2 = (width/2)+3,y2

    prob = PositionSearchProblem(gameState, start = position1, goal = position2)

    return len(search.breadthFirstSearch(prob, isRed, width))
